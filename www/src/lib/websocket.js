...
export class WS {
	...
	connect() {
		...
		this.ws.onmessage = (message) => {
			...
			} else if (Object.hasOwn(json, 'volume')) {
				volume.set(json.volume.volume);
			}
		};
	...
	volumeUp() {
		this.ws.send(JSON.stringify({ volumeUp: null }));
	}

	volumeDown() {
		this.ws.send(JSON.stringify({ volumeDown: null }));
	}

	setVolume(volume) {
		this.ws.send(JSON.stringify({ setVolume: { volume } }));
	}
