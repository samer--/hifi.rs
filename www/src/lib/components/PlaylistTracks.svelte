...
<div class="absolute w-full h-full flex flex-col bg-neutral-950 top-0 left-0">
	<div class="flex flex-row justify-between items-center py-4 bg-neutral-900 px-4">
		<h2 class="text-2xl xl:text-4xl pr-4">
			tracks in <span class="font-bold leading-none text-neutral-500">{$playlistTitle}</span>
		</h2>
		<div class="text-lg xl:text-2xl flex flex-row flex-nowrap gap-x-2">
			<button
				class="bg-blue-800 hover:bg-neutral-800 p-2"
				on:click={() => {
					show.set(null);
